...
def DFG_java(root_node, index_to_code, states):
    ...
    if (len(root_node.children) == 0 or root_node.type == 'string') and root_node.type != 'comment':
        ...
        # Extract the method name as a data node (for example: public String read(String url) {...} → "read")
        DFG = []
        name_node = root_node.child_by_field_name('name')
        if name_node is not None:
            name_indexes = tree_to_variable_index(name_node, index_to_code)
            for index in name_indexes:
                idx, code = index_to_code[index]
                # Record the method name as an independent data point
                DFG.append((code, idx, 'method_ident', [], []))
                states[f"METHOD::{code}"] = [idx]
        # Continue normal recursion to keep the original data flow intact
        for child in root_node.children:
            temp, states = DFG_java(child, index_to_code, states)
            DFG += temp
        return sorted(DFG, key=lambda x: x[1]), states
