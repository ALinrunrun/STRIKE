...
def get_example(code, tgt_word, substitute, lang = "java"):
    parser = parsers[lang]
    code = code.replace("\\n", "\n")
    parser = parsers[lang]
    tree = parser[0].parse(bytes(code, 'utf8'))
...
def get_code_tokens_treesitter_light(code: str, lang: str):
    ...
    code_tokens = [t for t in code_tokens if t.strip()]
    return code_tokens


def get_split_identifiers(code, lang, only_code_tokens=False):
    code = code.replace("\\n", "\n")
    try:
        code = remove_comments_and_docstrings(code, lang)
    except:
...
def change_code_style(source_code: str, lang: str, all_variable_name: list, code_style):
    ...
    _, source_code_tokens = get_identifiers(source_code, lang)
    used_identifiers = []
