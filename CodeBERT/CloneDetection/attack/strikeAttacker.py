...
print(f"STATEMENT_CANS_NUM              : {STATEMENT_CANS_NUM}") #1
print(f"WORD_CANS_NUM                   : {WORD_CANS_NUM}") #3
print(f"NUMBER_1                        : {NUMBER_1}") #2
print(f"NUMBER_2                        : {NUMBER_2}") #2
print("============================")
...
class Strike_Attacker(object):
    def __init__(self, args, model_tgt, tokenizer_tgt, tokenizer_mlm, model_mlm, tokenizer_llm, model_llm, fasttext_model, generated_substitutions):
        self.args = args
    ...
    def adaptive_beam_search(self, example, words_2, base_prob, base_code, alt_lists, chunks, init_k=STRUC_BEAM):
        orig_label = example[1].item()
        
        ori_pos={}
        for i, snippet_list in enumerate(alt_lists):
            # print(i)
            # for j in snippet_list[1]:
            #     print("===========")
            #     print(j)
        ...
        best_prob, best_code = base_prob, base_code
        best_path = []
        ...
        while beam:
            ...
            for _, code, pos, path in beam[:beam_width]:
                for i, snippet_list in enumerate(alt_lists):
                    ...
                    for new_snippet in snippet_list[1]:
                        ...
                        # print(temp_code)
                        # print("-------------------")
                        new_feature = " ".join(temp_code.split())
                        new_feature = self.tokenizer_tgt.tokenize(new_feature)
                        new_feature = convert_examples_to_features(new_feature,
                                                                words_2,
                                                                example[1].item(),
                                                                None, None,
                                                                self.tokenizer_tgt,
                        ...
                        all_temp_codes.append(new_feature)
                        meta.append((path, i, temp_code, temp_delta, pos))
            
            if len(all_temp_codes) == 0:
                return {
                    "success": False,
                    "code": best_code,
                    "prob": best_prob,
            ...
            new_dataset = CodeDataset(all_temp_codes)
            logits, preds = self.model_tgt.get_results(new_dataset, self.args.eval_batch_size)
            
            for index, temp_prob in enumerate(logits):
                temp_label = preds[index]
                path, i, temp_code, temp_delta, pos = meta[index]
                if int(temp_label) != int(orig_label):
                    # print("attacker predict label: ", temp_label)
                    return {
                        "success": True,
                        "code": temp_code,
                        "prob": temp_prob[orig_label],
                        "path": path + [i],
                    }
                
                if temp_prob[orig_label] < best_prob:
                    improved = True
                    best_prob, best_code = temp_prob[orig_label], temp_code
                    best_path = path + [i]
                    # print("=================================")
                    # print(best_path)
                    # print(best_code)
                ...
                new_candidates.append((temp_prob[orig_label], temp_code, new_pos, path + [i]))
            ...
            if improved:
                ...
                miss_rounds += 1
                if miss_rounds >= STRUC_EARLY_STOP:
    ...
    def greedy_search_idents(self, example, words_2, base_prob, base_code, alt_lists):
        orig_label = example[1].item()
        greedy = False
        candidate = None
        remove_ident = None
        ...
        for item, candidates in alt_lists.items():
            ...
            for new_candidate in candidates:
                ...
                new_feature = convert_examples_to_features(new_feature,
                                                        ...
                                                        self.args, None)
                all_temp_codes.append(new_feature)
                meta.append((item, new_candidate, temp_code))
        ...
        for index, temp_prob in enumerate(logits):
            temp_label = preds[index]
            item, new_candidate, temp_code = meta[index]
            if int(temp_label) != int(orig_label):
                greedy = True
                return {
                    "greedy": greedy,
    ...
    def strike_attack(self, orig_prob, topn_label, example, code_pair, selected_levels):
        code_1 = code_pair[2]
        code_2 = code_pair[3]
        ...
        for level_of_prompt in selected_levels:
            ...
            if level_of_prompt in (0, 1, 2):           
                ...
                result = self.adaptive_beam_search(
                    example, words_2, cur_prob, cur_code, res_lists, chunks
                )
                path = result["path"]
                ...
                for i in np.random.choice(self.substitutions[str(topn_label)], size=len(self.substitutions[str(topn_label)]), replace=False):
                    ...
                    if len(random_subs) >= NUMBER_1:
                        break
                # for k, i in enumerate(random_subs):
                #     print(i["variable_name1"])
                #     if k >20:
                #         break 
                substituions = []
                ori_embeddings1 = get_embeddings(cur_code, variable_names1+function_names1, self.tokenizer_mlm, self.model_mlm)
                ori_embeddings2 = get_embeddings(code_2, [], self.tokenizer_mlm, self.model_mlm)
                ...
                for sub in random_subs:
                    embeddings1 = get_embeddings(sub['code1'], [], self.tokenizer_mlm, self.model_mlm)
                    embeddings2 = get_embeddings(sub['code2'], [], self.tokenizer_mlm, self.model_mlm)
                    ...
                    cos_1 = np.sum(cos(ori_embeddings1, embeddings1).cpu().numpy()) / embeddings_leng
                    cos_2 = np.sum(cos(ori_embeddings2, embeddings2).cpu().numpy()) / embeddings_leng
                    cos_d = np.sign(cos_1 * cos_2) * np.sqrt(abs(cos_1 * cos_2))
                ...
                early_stop_flag = 0
                max_fail_streak = IDENT_EARLY_STOP
                while unconfir_snippets > 0:
                    result = self.greedy_search_idents(example, words_2, cur_prob, cur_code, substitutes)
