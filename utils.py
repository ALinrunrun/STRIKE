...
import numpy as np
import csv
from python_parser.run_parser import get_example, get_example_batch, remove_comments_and_docstrings, get_code_tokens_treesitter_light, extract_dataflow
...
def get_code_tokens(code, lang="java"):
    code = remove_comments_and_docstrings(code, lang)
    # tokens = javalang.tokenizer.tokenize(code)
    code_tokens = get_code_tokens_treesitter_light(code, lang)
    return code_tokens

def get_code_tokens_dataset(code, lang="java"):
    _, _, res = extract_dataflow(code, lang)
    return res
...
def _tokenize(seq, tokenizer):
    ...
    index = 0
    for word in words:
        # Instead of tokenizing the whole sentence directly, each split word is tokenized separately.
