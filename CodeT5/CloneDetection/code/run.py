...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        url_to_code={}
        with open('/Dataset/CD/BCB/adv_plus_set/codebert/data_codebert_attack_strike_all.jsonl') as f:
            for line in f:
                line=line.strip()
                js=json.loads(line)
...
def evaluate(args, model, tokenizer, prefix="",pool=None,eval_when_training=False):
    ...
    best_threshold=0
    best_f1=0
    # Determine the best_threshold on the validation set.
    ...
    # Use the best_threshold to compute the metrics.
...
def ASR(args, model, tokenizer,pool=None):
    # Load clean samples
    eval_dataset = load_and_cache_examples(args, tokenizer, test=True,pool=pool)
    ...
    for i, (example) in enumerate(eval_dataloader):
        ...
        # Continue only if the sample was originally classified correctly.
        if pred_label == label:
            total_num += 1
            url1, url2, code1, code2 = code_pairs[i]
            adv_inputs_ids = convert_advs_to_features(adv_codes[i], code2, tokenizer, args)
            adv_inputs_ids = torch.tensor(adv_inputs_ids).unsqueeze(0).to(args.device)
            # print(adv_inputs_ids.shape)
            with torch.no_grad():
                adv_logit = model(adv_inputs_ids)
            adv_logit = adv_logit.cpu().numpy()
            adv_pred_label = 0 if (adv_logit[:,0]>0.5)[0] else 1
            #The attack is successful if the adversarial sample is misclassified
...
def similarity(args, model, tokenizer,config,pool=None):
    #Load clean samples
    eval_dataset = load_and_cache_examples(args, tokenizer, test=True,pool=pool)
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=1)
    ...
    for i, (example) in enumerate(eval_dataloader):
        ...
        # Continue only if the sample was originally classified correctly.
        if pred_label == label:
            pre_trained_embedding = model_sub(input_id)
...
def main():
    ...
    if args.do_asr and args.local_rank in [-1, 0]:
        ...
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
        result = ASR(args, model, tokenizer,pool=pool)
        logger.info("***** Attack results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(round(result[key],4)))
