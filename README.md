...
- [Experiments](#experiments)
    ...
    - [Model Fine-tuning and Evaluation](#model-fine-tuning-and-evaluation)
...
We prepare a subset S covering TEST SET for robustness verification, and split it into S1 (indices 0–half) and S2 (indices half–all). You can use the split tool `split_testset.py` to extract these subsets.
...
```
The first script, get_ori_plus_adv_set.py, generates a combined training set for each method and model by merging the original training data with adversarial samples produced by that method on the corresponding model (samples indexed from 0–499 are used).

The second script, get_adv_set.py, generates adversarial test sets, which contain only the successfully attacked samples for each tool and model.
